...
create user subconsultas_jardineria identified by subconsultas_jardineria;
GRANT CONNECT, RESOURCE, DBA TO subconsultas_jardineria;
*/
...
INSERT INTO cliente VALUES (32,'france telecom','FraÃƒÂ§ois','Toulou','(33)5120578961','(33)5120578961','6 place d Alleray 15ÃƒÂ¨me',NULL,'Paris',NULL,'France','75010',16,10000);
...
--Consultas multitabla (composicion interna)
	...
	SELECT c.nombre_cliente Nombre_cliente, e.nombre Representante 
	...
	SELECT c.nombre_cliente, e.nombre || ' ' || e.apellido1 || ' ' || e.apellido2 AS "Representante", o.ciudad 
	...
	SELECT o.linea_direccion1 || ' ' || o.linea_direccion2 AS "Direccion oficina"
	FROM oficina o, empleado e, cliente c
	WHERE o.codigo_oficina = e.codigo_oficina 
	AND e.codigo_empleado = c.codigo_empleado_rep_ventas 
	AND (UPPER(c.linea_direccion1) LIKE '%FUENTELABRADA%' 
	OR UPPER(c.linea_direccion2) LIKE '%FUENTELABRADA%');

    --7 Devuelve el nombre de los clientes y el nombre de sus representantes junto con la ciudad 
    --de la oficina a la que pertenece el representante.
	SELECT DISTINCT c.nombre_cliente Cliente, e.nombre || ' ' || e.apellido1 || ' ' || e.apellido2 AS "Representante", o.ciudad Ciudad
	FROM cliente c, empleado e, oficina o 
	WHERE o.codigo_oficina = e.codigo_oficina
	AND e.codigo_empleado = c.codigo_empleado_rep_ventas;

    --8 Devuelve un listado con el nombre de los empleados junto con el nombre de sus jefes.
	SELECT CONCAT(e.nombre, CONCAT(' ', CONCAT(e.apellido1, CONCAT(' ', e.apellido2)))) AS Empleado, CONCAT(e2.nombre, CONCAT(' ', CONCAT(e2.apellido1, CONCAT(' ', e2.apellido2)))) AS Jefe
	FROM empleado e, empleado e2
	WHERE e2.codigo_jefe = e.codigo_empleado;			
	...
	SELECT DISTINCT gp.gama 
	FROM cliente c, pedido p, detalle_pedido dp, producto p2, gama_producto gp
	WHERE c.codigo_cliente = p.codigo_cliente 
...
--Consultas multitabla (composicion externa)
    ...
    --3 Devuelve un listado que muestre los clientes que no han realizado ningun pago y 
    --los que no han realizado ningun pedido.
	...
	AND p2.codigo_pedido IS NULL;

    --8 Devuelve las oficinas donde no trabajan ninguno de los empleados que hayan sido los 
    --representantes de ventas de algun cliente que haya realizado la compra de algun 
    ...
    --9 Devuelve un listado con los clientes que han realizado algun pedido, pero no han 
    --realizado ningun pago.
	SELECT DISTINCT c.codigo_cliente 
	FROM cliente c, pago p, pedido p2 
	WHERE c.codigo_cliente = p2.codigo_pedido 
	AND c.codigo_cliente = p.codigo_cliente(+)
	AND p.codigo_cliente IS NULL;

    --10 Devuelve un listado con los datos de los empleados que no tienen clientes asociados 
    --y el nombre de su jefe asociado.
	SELECT DISTINCT e.codigo_empleado, e.nombre || ' ' || e.apellido1 || ' ' || e.apellido2 AS "Empleado", e2.nombre || ' ' || e2.apellido1 || ' ' || e2.apellido2 AS "Jefe"
	FROM empleado e, empleado e2, cliente c
	WHERE e.codigo_empleado = c.codigo_empleado_rep_ventas(+) 
	AND c.codigo_empleado_rep_ventas IS NULL 
	AND e.codigo_jefe = e2.codigo_empleado;

--Consultas resumen
    --1 Cuantos empleados hay en la compania?
	SELECT COUNT(*) 
	FROM empleado;

    --2 Cuantos clientes tiene cada pais?
	SELECT pais, COUNT(codigo_cliente)
	FROM cliente
	GROUP BY pais;

    --3 Cual fue el pago medio en 2009?
	SELECT AVG(total) 
	FROM pago 
	WHERE EXTRACT(YEAR FROM fecha_pago) = 2009;

    --4 Cuantos pedidos hay en cada estado? Ordena el resultado de forma descendente por el numero de pedidos.
	SELECT estado, COUNT(estado) 
	FROM pedido
	GROUP BY estado
	ORDER BY COUNT(estado) DESC;			
	
    --5 Calcula el precio de venta del producto mas caro y mas barato en una misma consulta.
	SELECT MAX(NVL(precio_venta, 0)) AS "Maximo precio venta", 
		   MIN(NVL(precio_venta, 0)) AS "Minimo precio venta"
	FROM producto;
	
    --6 Calcula el numero de clientes que tiene la empresa.
	SELECT COUNT(*)
	FROM cliente;

    --7 Cuantos clientes tiene la ciudad de Madrid?
	...
	SELECT COUNT(*) AS "Clientes ciudad con M"
	FROM cliente 
	WHERE UPPER(ciudad) LIKE 'M%';

    --9 Devuelve el codigo de empleado y el numero de clientes al que atiende cada representante de ventas.
	SELECT e.codigo_empleado, COUNT(c.codigo_cliente)
	FROM empleado e, cliente c
	WHERE e.codigo_empleado = c.codigo_empleado_rep_ventas
	GROUP BY e.codigo_empleado;

    --10 Calcula el numero de clientes que no tiene asignado representante de ventas.
    SELECT COUNT(c.codigo_cliente)
	FROM cliente c, empleado e 
	WHERE c.codigo_empleado_rep_ventas(+) = e.codigo_empleado
	AND c.codigo_empleado_rep_ventas IS NULL;

    --11 Calcula la fecha del primer y ultimo pago realizado por cada uno de los clientes.
	SELECT MIN(NVL(fecha_pago, 0)) AS "Primer pago", MAX(NVL(fecha_pago, 0)) AS "Ultimo pago"
	FROM pago;						--Esta mal. Fechas.
	
    --12 Calcula el numero de productos diferentes que hay en cada uno de los pedidos.
	SELECT COUNT(p.codigo_producto), p.nombre
	FROM producto p, detalle_pedido dp, pedido p2 
	WHERE p.codigo_producto = dp.codigo_producto 
	AND dp.codigo_pedido = p2.codigo_pedido
	GROUP BY p.nombre;
    ...
    --14 Devuelve un listado de los 20 codigos de productos mas vendidos y el numero total de 
    --unidades que se han vendido de cada uno. El listado debera estar ordenado por el numero 
    --total de unidades vendidas.
	SELECT p.codigo_producto, dp.cantidad
	FROM producto p, detalle_pedido dp 
	WHERE p.codigo_producto = dp.codigo_producto
	ORDER BY dp.cantidad DESC, p.codigo_producto DESC;

    --15 La facturacion que ha tenido la empresa en toda la historia, indicando la base imponible, 
    --el IVA y el total facturado. La base imponible se calcula sumando el coste del producto por 
    --el numero de unidades vendidas de la tabla detalle_pedido. El IVA es el 21 % de la base 
    --imponible, y el total la suma de los dos campos anteriores.
	SELECT SUM((p.precio_venta * dp.cantidad)) AS "Base imponible", 
		   SUM(((p.precio_venta * dp.cantidad) * 0.21)) AS "IVA",
		   SUM((((p.precio_venta * dp.cantidad) * 0.21) + ((p.precio_venta * dp.cantidad)))) AS "Total"
	FROM producto p, detalle_pedido dp
	WHERE p.codigo_producto = dp.codigo_producto;

    --16 La misma informacion que en la pregunta anterior, pero agrupada por codigo de producto.
	SELECT p.codigo_producto,
	...
	--17 La misma informacion que en la pregunta anterior, pero agrupada por codigo de producto 
    --filtrada por los codigos que empiecen por OR.
	SELECT p.codigo_producto,
		   SUM((p.precio_venta * dp.cantidad)) AS "Base imponible", 
		   SUM(((p.precio_venta * dp.cantidad) * 0.21)) AS "IVA",
		   SUM((((p.precio_venta * dp.cantidad) * 0.21) + ((p.precio_venta * dp.cantidad)))) AS "Total"
	FROM producto p, detalle_pedido dp
	WHERE p.codigo_producto = dp.codigo_producto
	AND UPPER(p.codigo_producto) LIKE 'OR%'
	GROUP BY p.codigo_producto;

    --18 Lista las ventas totales de los productos que hayan facturado mas de 3000 euros. 
    --Se mostrara el nombre, unidades vendidas, total facturado y total facturado con impuestos (21% IVA).
	SELECT p.nombre, dp.cantidad,
		   SUM((p.precio_venta * dp.cantidad)) AS "Total facturado", 
		   SUM((((p.precio_venta * dp.cantidad) * 0.21) + ((p.precio_venta * dp.cantidad)))) AS "Total facturado m�s IVA"
	FROM producto p, detalle_pedido dp
	WHERE p.codigo_producto = dp.codigo_producto
...
--Subconsultas
    ...
    --2 Devuelve el nombre, apellido1 y cargo de los empleados que no representen a ningun cliente.
	SELECT e.nombre || ' ' || e.apellido1 AS "Empleado", e.puesto AS "Cargo"
	FROM empleado e
	WHERE e.codigo_empleado NOT IN
						(SELECT DISTINCT e.codigo_empleado_rep_ventas
						FROM cliente);
    ...
    --3 Devuelve el nombre del producto que tenga el precio de venta mas caro.
	SELECT nombre 
	FROM producto 
	WHERE precio_venta =
    ...
    --4 Devuelve el nombre del producto del que se han vendido mas unidades. (Ten en cuenta que 
    --tendras que calcular cual es el numero total de unidades que se han vendido de cada producto 
    --a partir de los datos de la tabla detalle_pedido. Una vez que sepas cual es el codigo del 
    --producto, puedes obtener su nombre facilmente.)
	SELECT lp.* 
    ...
    --5 Los clientes cuyo limite de credito sea mayor que los pagos que haya realizado.
	SELECT c.nombre_cliente 
	FROM cliente c
	WHERE c.limite_credito > 
				ANY	(SELECT SUM(p.total)
					FROM pago p
					WHERE c.codigo_cliente = p.codigo_cliente);
				
    --6 El producto que mas unidades tiene en stock y el que menos unidades tiene.
	SELECT nombre, cantidad_en_stock
	FROM producto 
	WHERE cantidad_en_stock = 
				 (SELECT MAX(cantidad_en_stock)
				 FROM producto)
	OR cantidad_en_stock = 
...
--Consultas variadas
    --1 Devuelve el listado de clientes indicando el nombre del cliente y cuantos pedidos ha realizado.
	--Tenga en cuenta que pueden existir clientes que no han realizado ningun pedido.
	SELECT c.codigo_cliente, c.nombre_cliente, COUNT(p.codigo_pedido) 
	FROM cliente c, pedido p 
	WHERE c.codigo_cliente = p.codigo_cliente(+)
    ...
    --Tenga en cuenta que pueden existir clientes que no han realizado ningun pago.
	SELECT c.nombre_cliente, p.total 
	FROM cliente c, pago p
	WHERE c.codigo_cliente = p.codigo_cliente(+);

    --3 Devuelve el nombre de los clientes que hayan hecho pedidos en 2008 ordenados alfabeticamente de menor a mayor.
	SELECT c.nombre_cliente 
	FROM cliente c, pedido p 
	WHERE c.codigo_cliente = p.codigo_cliente 
    ...
    --4 Devuelve el nombre del cliente, el nombre y primer apellido de su representante de ventas y el numero de 
    --telefono de la oficina del representante de ventas, de aquellos clientes que no hayan realizado ningun pago.
	SELECT c.nombre_cliente, e.nombre || ' ' || e.apellido1 AS "Representante de ventas", o.telefono 
    ...
    --representante de ventas y la ciudad donde esta su oficina.
	SELECT c.nombre_cliente AS "Cliente", e.nombre || ' ' || e.apellido1 AS "Representante de ventas", o.ciudad
	FROM cliente c, empleado e, oficina o 
	WHERE c.codigo_empleado_rep_ventas = e.codigo_empleado
	AND e.codigo_oficina = o.codigo_oficina;
	
    --6 Devuelve el nombre, apellidos, puesto y telefono de la oficina de aquellos empleados que no sean 
    --representante de ventas de ningun cliente.
	SELECT e.nombre || ' ' || e.apellido1 || ' ' || e.apellido2 AS "Empleado", o.telefono 
	FROM empleado e, oficina o, cliente c
	WHERE e.codigo_oficina = o.codigo_oficina 
	AND e.codigo_empleado = c.codigo_empleado_rep_ventas(+)
	AND c.codigo_empleado_rep_ventas IS NULL;

    --7 Devuelve un listado indicando todas las ciudades donde hay oficinas y el numero de empleados que tiene.
